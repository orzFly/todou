...
- A bare current cursor on stdout: `todou watch -p <proj> --poll --print-cursor`, or
  `todou issue watch <n> -p <proj> --poll --print-cursor` for one card. It exits 0 with or without
  news and conflicts with `--json`. Use it only for a wait that no write precedes. On a card that has
  moved, the single-card form exits 1 with an empty stdout rather than handing back a cursor that
  belongs to the project the card landed in.
