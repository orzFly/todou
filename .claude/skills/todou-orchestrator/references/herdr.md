...
command forms only; the decisions — when to dispatch, to whom, what goes in the brief, when to retire
— stay in `SKILL.md`. herdr's own complete reference is `herdr --skill`.
...
returned. `working` means a process is running, and a blocked wait is a running process too; a
`--forever` wait prints a heartbeat to stderr, so a terminal with no heartbeat and no new output for
hours is stuck whatever the state says.
