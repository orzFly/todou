...
import type { issueListDescriptorOf } from "@/api/issues-cache.ts";

export type QueryKeyLike = ReadonlyArray<unknown>;

/**
 * Read-only cache access shared by the page and worker rule evaluators.
 * getQueriesData matches a key prefix and preserves cache iteration order;
 * getQueryData reads an exact key. Missing/evicted values remain undefined.
 * QueryClient satisfies this interface without a page-specific adapter.
 */
export interface CacheView {
  getQueriesData(filters: {
    queryKey: QueryKeyLike;
  }): Array<[QueryKeyLike, unknown]>;
...
export function cachedIssueRow(
  ...
  for (const [, data] of cache.getQueriesData({ queryKey: key })) {
...
export function statusCategories(
  ...
  const statuses = cache.getQueryData(["statuses", slug]);
